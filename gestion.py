...
def conectar_google():
    ...
    # Lógica para detectar si estamos en local o en la nube (Streamlit Cloud)
    if "gcp_service_account" in st.secrets:
        creds = Credentials.from_service_account_info(st.secrets["gcp_service_account"], scopes=scope)
    else:
        # Reemplaza 'llave_google.json' por el nombre exacto de tu archivo descargado
...
def cargar_datos():
    try:
        ...
        c = pd.DataFrame(ws_c.get_all_records())
        
        # Cargar Viajes
        ws_v = sh.worksheet("viajes")
        v = pd.DataFrame(ws_v.get_all_records())
        v['Importe'] = pd.to_numeric(v['Importe'], errors='coerce').fillna(0)
...
def guardar_datos(nombre_hoja, df):
    try:
        ...
        ws.clear()
        # Preparamos los datos: encabezados + valores (convirtiendo fechas a texto para evitar errores)
...
if sel == "CLIENTES":
    ...
    elim_c = st.selectbox("Eliminar Cliente:", ["Seleccione..."] + list(st.session_state.clientes['Razón Social']))
    if st.button("ELIMINAR CLIENTE"):
        if elim_c != "Seleccione...":
    ...
    if st.session_state.clientes.empty: st.warning("Cargue clientes primero.")
    else:
        with st.form("f_viaje"):
            ...
            if st.form_submit_button("GUARDAR VIAJE"):
                ...
                st.success("Viaje guardado en la nube.")
    ...
    with st.form("f_nc"):
        ...
        if st.form_submit_button("REGISTRAR AJUSTE"):
            if nro_afip and monto > 0:
                ...
                guardar_datos("viajes", st.session_state.viajes)
                st.success("Ajuste registrado en la nube.")
    ...
    if not st.session_state.clientes.empty:
        ...
        c1.metric("SALDO", f"$ {df_ind['Importe'].sum():,.2f}")
        if not df_ind.empty:
            c2.download_button("📥 PDF", generar_pdf_ctacte(cl, df_ind), f"CtaCte_{cl}.pdf")
    ...
    st.header("📜 Historial y Eliminación de Comprobantes")
    if st.session_state.viajes.empty:
        st.info("No hay comprobantes cargados.")
    else:
        viajes_reverse = st.session_state.viajes.iloc[::-1]
        for i, row in viajes_reverse.iterrows():
            with st.container():
                ...
                if col3.button("🗑️ BORRAR", key=f"btn_borrar_{i}"):
