...
def cargar_datos():
    ...
    # MODIFICACIÓN: Se agregan Alias y CBU a la estructura
...
if sel == "CALENDARIO":
    ...
    with st.expander("➕ ALTA DE NUEVO PROVEEDOR", expanded=False):
        with st.form("f_prov", clear_on_submit=True):
            ...
            # MODIFICACIÓN: Nuevos campos Alias y CBU
            c3, c4 = st.columns(2)
            alias = c3.text_input("Alias CBU")
            cbu = c4.text_input("CBU (22 dígitos)")
            
            if st.form_submit_button("REGISTRAR PROVEEDOR"):
                if rs and doc:
                    # Se incluye alias y cbu en el guardado
                    np = pd.DataFrame([[rs, doc, cuenta, cat_iva, alias, cbu]], columns=st.session_state.proveedores.columns)
    ...
    if not st.session_state.proveedores.empty:
        for i, row in st.session_state.proveedores.iterrows():
            with st.container():
                ...
                # MODIFICACIÓN: Mostrar Alias y CBU en la info
                c_inf.caption(f"📂 Cuenta: {row['Cuenta de Gastos']} | {row['Categoría IVA']} | 🏦 Alias: {row.get('Alias','-')} | CBU: {row.get('CBU','-')}")
                ...
                if st.session_state.get(f"edit_p_mode_{i}", False):
                    with st.form(f"f_edit_p_{i}"):
                        ...
                        # MODIFICACIÓN: Campos de edición para Alias y CBU
                        n_alias = ce1.text_input("Alias", value=row.get('Alias',''))
                        n_cbu = ce2.text_input("CBU", value=row.get('CBU',''))
                        if st.form_submit_button("✅ Guardar"):
                            st.session_state.proveedores.loc[i] = [n_rs, n_doc, row['Cuenta de Gastos'], row['Categoría IVA'], n_alias, n_cbu]
                            guardar_datos("proveedores", st.session_state.proveedores)
    ...
    # MODIFICACIÓN: Cruzar datos de compras con info bancaria del proveedor
    if not st.session_state.compras.empty:
        res_p = st.session_state.compras.groupby('Proveedor')['Total'].sum().reset_index()
        
        if not st.session_state.proveedores.empty:
            info_bancaria = st.session_state.proveedores[['Razón Social', 'Alias', 'CBU']]
            res_p = res_p.merge(info_bancaria, left_on='Proveedor', right_on='Razón Social', how='left').drop(columns=['Razón Social'])
        
        # Reordenar columnas para visualización clara
        cols_order = ['Proveedor', 'Total', 'Alias', 'CBU']
        res_p = res_p[[c for c in cols_order if c in res_p.columns]]
        
        st.table(res_p.style.format({"Total": "$ {:,.2f}"}))
    else:
        st.info("No hay movimientos de compras registrados.")
