...
def cargar_datos():
    ...
    col_t = ["Fecha", "Tipo", "Concepto", "Monto", "Cuenta", "AFIP_Asoc"] # Nueva columna Tesorería
    try:
        sh = conectar_google()
            ...
            with st.container():
                ...
                if st.session_state.get(f"edit_mode_{i}", False):
                    with st.form(f"f_edit_{i}"):
                        ...
                        if ce1.form_submit_button("✅ Guardar"):
...
# --- NUEVO MÓDULO TESORERIA ---
    ...
    for i, cta in enumerate(cuentas):
        ...
        cols_s[i].metric(cta, f"$ {ing - egr:,.2f}")

    st.divider()
    t1, t2 = st.tabs(["📥 Registrar Movimiento", "📊 Historial de Caja"])
    
    with t1:
        with st.form("form_t", clear_on_submit=True):
            c1, c2 = st.columns(2)
            t_mov = c1.selectbox("Tipo", ["INGRESO", "EGRESO"])
            cta_m = c2.selectbox("Cuenta / Banco", cuentas)
            ...
            afip_asoc = st.text_input("Asociar Comprobante AFIP (NC/ND)")
            if st.form_submit_button("REGISTRAR"):
                nueva_t = pd.DataFrame([[date.today(), t_mov, concep, monto_m, cta_m, afip_asoc]], columns=df_t.columns)
                st.session_state.tesoreria = pd.concat([st.session_state.tesoreria, nueva_t], ignore_index=True)
                guardar_datos("tesoreria", st.session_state.tesoreria); st.success("Movimiento guardado"); st.rerun()
    with t2:
