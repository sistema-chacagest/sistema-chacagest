...
def cargar_datos():
    ...
    try:
        ...
        # Cargar o crear hoja de presupuestos
        try:
...
def guardar_datos(nombre_hoja, df):
    try:
        ...
        try:
            ...
            # Si no existe la hoja (como presupuestos), la crea
...
if sel == "CALENDARIO":
    ...
    custom_css = ".fc-button-primary { background-color: #5e2d61 !important; border-color: #5e2d61 !important; color: white !important; } .fc-event { cursor: pointer; }"
    ...
    if st.session_state.viaje_ver is not None:
        ...
        if idx in st.session_state.viajes.index:
            ...
            <div style="background-color: #f0f2f6; padding: 15px; border-left: 5px solid #f39c12; border-radius: 5px; margin-top: 20px;">
                ...
                <p><b>Cliente:</b> {v_det['Cliente']} | <b>Móvil:</b> {v_det['Patente / Móvil']}<br>
                <b>Ruta:</b> {v_det['Origen']} ➔ {v_det['Destino']} | <b>Importe:</b> $ {v_det['Importe']}</p>
            </div>
            """, unsafe_allow_html=True)
    ...
    with st.expander("📝 CREAR NUEVO PRESUPUESTO", expanded=True):
        with st.form("f_presu", clear_on_submit=True):
            ...
            c1, c2 = st.columns(2)
            fecha_v_p = c1.date_input("Fecha Viaje")
            fecha_venc = c2.date_input("Fecha Vencimiento Presupuesto", value=date.today() + timedelta(days=15))
            ...
            c5, c6 = st.columns(2)
            tipo_vh = c5.selectbox("Tipo de Vehículo", ["Camioneta", "Chasis", "Balancín", "Semi-Remolque", "Acoplado", "Furgón"])
            monto_p = c6.number_input("Importe Presupuestado $", min_value=0.0)
            
            if st.form_submit_button("REGISTRAR PRESUPUESTO"):
                new_p = pd.DataFrame([[cliente_p, fecha_v_p, orig_p, dest_p, tipo_vh, monto_p, fecha_venc]], 
                                    columns=["Cliente", "Fecha Viaje", "Origen", "Destino", "Tipo Vehículo", "Importe", "Fecha Vencimiento"])
    ...
    st.subheader("📋 Presupuestos Realizados")
    ...
    with st.expander("➕ ALTA DE NUEVO CLIENTE"):
        with st.form("f_cli", clear_on_submit=True):
            c1, c2 = st.columns(2)
            r = c1.text_input("Razón Social *"); cuit = c2.text_input("CUIT / CUIL *")
