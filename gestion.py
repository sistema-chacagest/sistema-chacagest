...
if sel == "CALENDARIO":
    ...
    if st.session_state.viaje_ver is not None:
        ...
        if idx in st.session_state.viajes.index:
            ...
            st.markdown(f"""<div style="background-color: #f0f2f6; padding: 15px; border-left: 5px solid #f39c12; border-radius: 5px; margin-top: 20px;"> <h4 style="color: #5e2d61; margin: 0;">Detalles</h4><p><b>Cliente:</b> {v_det['Cliente']}</p><p><b>Ruta:</b> {v_det['Origen']} ➔ {v_det['Destino']}</p> <p><b>Importe:</b> $ {v_det['Importe']}</p></div>""", unsafe_allow_html=True)
        ...
        for i, row in st.session_state.clientes.iterrows():
            with st.container():
                ...
                if c_el.button("🗑️", key=f"del_cli_{i}"):
                    if not st.session_state.viajes[st.session_state.viajes['Cliente'] == row['Razón Social']].empty: st.error("No se puede eliminar: tiene viajes asociados.")
                    else: st.session_state.clientes = st.session_state.clientes.drop(i).reset_index(drop=True); guardar_datos("clientes", st.session_state.clientes); st.rerun()
    ...
    with tab_crear:
        with st.form("f_presu", clear_on_submit=True):
            ...
            p_f_emi = st.date_input("Fecha Emisión", date.today())
            p_det = st.text_area("Detalle")
            p_imp = st.number_input("Importe Total $", min_value=0.0)
            if st.form_submit_button("GENERAR"):
                nuevo_p = pd.DataFrame([[p_f_emi, p_cli, date.today(), p_det, "Bus", p_imp]], columns=st.session_state.presupuestos.columns)
    ...
    with tab_historial: st.dataframe(st.session_state.presupuestos)
...
# --- MÓDULOS DE TESORERIA (PEDIDOS) ---
    ...
    with st.form("f_teso_v"):
        ...
        afip = st.text_input("Asociar Comprobante AFIP (Opcional)")
        if st.form_submit_button("REGISTRAR"):
            ...
            guardar_datos("tesoreria", st.session_state.tesoreria); st.success("Registrado en Tesorería"); st.rerun()

elif sel == "COBRANZA DE VIAJE":
    st.header("💸 Cobranza de Viaje")
    with st.form("f_cob"):
        cl_cob = st.selectbox("Seleccionar Cliente", st.session_state.clientes['Razón Social'].unique())
        monto_cob = st.number_input("Monto Cobrado $", min_value=0.0)
        cta_cob = st.selectbox("Forma de Cobro", ["CAJA COTI", "CAJA TATO", "BANCO GALICIA", "BANCO PROVINCIA", "BANCO SUPERVIELLE"])
        afip_cob = st.text_input("Asociar Comprobante AFIP (Requerido)")
        if st.form_submit_button("REGISTRAR COBRANZA"):
            # Reste en Cta Cte
            nv = pd.DataFrame([[date.today(), cl_cob, date.today(), "COBRANZA", "CREDITO", "-", -monto_cob, "RECIBO", afip_cob]], columns=st.session_state.viajes.columns)
            st.session_state.viajes = pd.concat([st.session_state.viajes, nv], ignore_index=True)
            ...
            st.success("Cobro registrado y descontado de Cuenta Corriente."); st.rerun()
