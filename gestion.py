...
import base64
from fpdf import FPDF # Librería para generación de PDF
...
# =========================================================
# --- NUEVA LÓGICA: MOTOR DE GENERACIÓN PDF ---
# =========================================================

def crear_pdf_documento(titulo_doc, subtitulo, nombre_entidad, detalle, monto, ref_afip="", tipo="VENTA"):
    """Genera un archivo PDF binario para descarga directa"""
    pdf = FPDF()
    pdf.add_page()
    
    # Encabezado estilo Chacagest
    pdf.set_fill_color(94, 45, 97) # Morado Corporativo
    ...
    pdf.cell(0, 5, "Desde 1996 viajando con vos | Chacabuco, Bs. As.", ln=True)
    
    # Cuadro de Tipo de Documento
    pdf.set_xy(140, 10)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("Arial", "B", 12)
    pdf.cell(60, 10, titulo_doc, border=1, ln=True, align='C')
    pdf.set_xy(140, 20)
    pdf.set_font("Arial", "", 9)
    pdf.cell(60, 10, f"Fecha: {date.today()}", ln=True, align='R')
    
    # Cuerpo del documento
    pdf.set_text_color(0, 0, 0)
    pdf.set_y(45)
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, f"{'CLIENTE' if tipo == 'VENTA' else 'PROVEEDOR'}: {nombre_entidad}", ln=True)
    
    if subtitulo:
        pdf.set_font("Arial", "I", 11)
        pdf.cell(0, 8, f"Referencia: {subtitulo}", ln=True)
        
    pdf.ln(5)
    pdf.set_font("Arial", "B", 10)
    pdf.cell(0, 8, "DETALLE DEL CONCEPTO:", ln=True)
    pdf.set_font("Arial", "", 10)
    ...
    if ref_afip:
        ...
        pdf.cell(0, 7, f"Comprobante Asociado: {ref_afip}", ln=True)

    # Total Destacado
    pdf.ln(10)
    pdf.set_fill_color(240, 240, 240)
    pdf.set_font("Arial", "B", 14)
    pdf.cell(130, 12, "", ln=0)
    pdf.cell(60, 12, f"TOTAL: $ {abs(monto):,.2f}", border=1, ln=True, align='C', fill=True)
    
    # Pie de página / Leyendas
    pdf.set_y(-50)
    pdf.set_font("Arial", "I", 8)
    pdf.set_text_color(100, 100, 100)
    if titulo_doc == "PRESUPUESTO":
        pdf.multi_cell(0, 4, "• La seña para la reserva es del 30%.\n• Los gastos de los choferes (hospedaje y comida) estarán a cargo del contratante.")
    
    pdf.set_y(-20)
    pdf.set_font("Arial", "", 8)
    pdf.cell(0, 10, "Documento generado por Sistema CHACAGEST - Gestión de Transporte.", ln=True, align='C')
...
if sel == "CALENDARIO":
    ...
    for i, row in st.session_state.presupuestos.iterrows():
        ...
        pdf_data = crear_pdf_documento("PRESUPUESTO", row['Tipo Móvil'], row['Cliente'], row['Detalle'], row['Importe'])
        c2.download_button("📩 PDF", pdf_data, f"Presupuesto_{row['Cliente']}.pdf", "application/pdf", key=f"dl_{i}")
    ...
    with t1:
        with st.form("f_cob"):
            ...
            if st.form_submit_button("COBRAR"):
                ...
                pdf_rec = crear_pdf_documento("RECIBO DE PAGO", "Cobranza de Viaje", cli, "Recibimos el pago correspondiente a servicios de transporte.", mon, ref)
                st.download_button("📩 Descargar Recibo PDF", pdf_rec, f"Recibo_{cli}.pdf", "application/pdf")
    ...
    with t2:
        with st.form("f_op"):
            ...
            if st.form_submit_button("PAGAR"):
                nt = pd.DataFrame([[date.today(), "PAGO PROV", "CAJA", "Pago", prov, -mon, "-"]], columns=st.session_state.tesoreria.columns)
                ...
                pdf_op = crear_pdf_documento("ORDEN DE PAGO", "Pago a Proveedor", prov, "Egreso de caja por pago de facturas/gastos.", mon, "", tipo="COMPRA")
